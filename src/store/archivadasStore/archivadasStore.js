...
const initialState = {
  // Datos del Formulario de Archivadas (CRUD) - Usa modelo Preparacion con estado_modulo=0
  id: null,
  placa: '',
  departamento: '',
  municipio: '',
  tipo_vehiculo: '',
  estado: 'finalizado', // Estado por defecto para archivadas
  ...
  // Lista de trámites archivadas y control de UI
  tramites: [],
  history: [],
  isLoading: false,
  error: null,
...
export const archivadasStore = createSlice({
  name: 'archivadasStore',
  initialState,
  reducers: {
    ...
    loadForEditStore: (state, action) => {
      const archivada = action.payload;
      state.id = archivada.id;
      state.placa = archivada.placa;
      state.departamento = archivada.departamento;
      state.municipio = archivada.municipio;
      state.tipo_vehiculo = archivada.tipo_vehiculo;
      state.estado = archivada.estado;
      state.estado_tracker = archivada.estado_tracker;
      state.estado_detalle = archivada.estado_detalle || '';
      state.fecha_recepcion_municipio = archivada.fecha_recepcion_municipio || '';
      state.proveedor = archivada.proveedor_id || '';
      state.paquete = archivada.paquete || '';
      state.lista_documentos = archivada.lista_documentos || [];
      state.archivos = archivada.archivos || [];
      state.usuario = archivada.usuario || '';
      state.created_at = archivada.created_at;
      state.documentos_completos = archivada.documentos_completos || false;
      state.documentos_completados = archivada.documentos_completados || 0;
      state.total_documentos = archivada.total_documentos || 0;
      state.total_archivos = archivada.total_archivos || 0;
      state.hace_dias = archivada.hace_dias;
      state.codigo_encargado = archivada.codigo_encargado || '';
    },
...
export const {
  ...
  setLoading,
  setError,
} = archivadasStore.actions;

export default archivadasStore.reducer;
