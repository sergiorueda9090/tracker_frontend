...
import { getAllThunks, showThunk, deleteThunk } from '../../store/archivadasStore/archivadasThunks';
...
const MainTable = () => {
  ...
  const { tramites } = useSelector(state => state.archivadasStore);
  ...
  const handleDelete = (tramite) => {
    dispatch(showAlert({
      ...
      text: `¿Está seguro que desea eliminar el trámite archivado con placa "${tramite.placa}"? Esta acción es permanente y no se podrá deshacer.`,
  ...
  const handleVerTrazabilidad = (id) => {
    navigate(`/archivadas/history/${id}`);
  };
  ...
  return (
    <Paper className="page-paper">
      ...
      {/* Diálogo de Visualización de Archivos */}
      <Dialog
            ...
            <Box display="flex" alignItems="center" gap={2}>
              ...
              <Box>
                <Typography variant="h6" fontWeight={600} sx={{ fontSize: { xs: '1.1rem', md: '1.25rem' } }}>
                  Archivos del Trámite Archivado
                </Typography>
                {selectedTramite && (
                  ...
                  <List disablePadding>
                    {selectedTramite.archivos.map((archivo) => (
                      <Card
                        ...
                        sx={{
                          mb: 1.5,
                          border: selectedFile?.id === archivo.id ? '2px solid #6B7280' : '1px solid #e0e0e0',
                          bgcolor: selectedFile?.id === archivo.id ? 'rgba(107, 114, 128, 0.08)' : 'white',
