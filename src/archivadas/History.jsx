    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Obtener información del trámite archivado
                const tramiteResponse = await axios.get(
                    `${URL}/api/archivadas/${id}/`,
                    {
                        headers: { Authorization: `Bearer ${token}` }
                    }
                );

                // Obtener historial del trámite archivado
                const historyResponse = await axios.get(
                    `${URL}/api/archivadas/${id}/history/`,
                    {
                        headers: { Authorization: `Bearer ${token}` }
                    }
                );
