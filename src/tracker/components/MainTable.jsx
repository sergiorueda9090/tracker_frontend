...
const MainTable = () => {
  ...
  useEffect(() => {
    ...
    switch (type) {
      case 'tracker_created':
        // Agregar nuevo trámite a la lista en tiempo real
        console.log('✅ Nuevo trámite recibido del módulo Preparación:', data);
        dispatch(addTramiteRealtime(data));

        // Mostrar notificación visual
        Swal.fire({
          icon: 'success',
          title: '🚀 Nuevo trámite recibido',
          text: `La placa ${data.placa} ha sido enviada desde Preparación`,
          toast: true,
          position: 'top-end',
      ...
      case 'tracker_deleted':
        // Estos se manejan en otros lugares
        break;
  ...
  const getEstadoTrackerChip = (estado) => {
    const estados = {
      'sin_tracker': {
        label: 'Sin Tracker',
        color: 'default',
    ...
    const estadoConfig = estados[estado] || estados['sin_tracker'];
          ...
          <TableBody>
            {tramites.map((tramite) => (
                <TableRow key={tramite.id} className="table-row">
                  ...
                  <TableCell align="center">
                    {getEstadoTrackerChip(tramite.estado_tracker)}
                  </TableCell>
